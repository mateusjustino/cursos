...
soundPoints = pygame.mixer.Sound(os.path.join(directorySound, 'score_sound.wav'))
soundPoints.set_volume(1)
...
def restart():
    global points, gameSpeed, collided, choiceObstacle
    points = 0
    gameSpeed = 10
    collided = False
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                if dino.rect.y == dino.posYStart and collided == False: # aqui só posso pular se o dino estiver no chao
                    dino.toJump()

            if event.key == pygame.K_r and collided == True:
                restart()
